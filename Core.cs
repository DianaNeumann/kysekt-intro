...
namespace sekta
{
    class Core
    {
        ...
        {
            int indexToDelete = taskList.FindIndex(task => task.ID == id);
        ...
        private static void CompleteTask(int id)
        {
            try
            {
                int indexToComplete = taskList.FindIndex(task => task.ID == id);
                taskList[indexToComplete].isCompleted = true;
            }
            catch(Exception ex)
            {
        ...
        {
            taskList[taskList.FindIndex(task => task.ID == idDate)].Date = oDate.ToString().Substring(0, 10);
        ...
        {
            taskList[taskList.FindIndex(task => task.ID == TaskId)].AddSubTask(SubTaskName);
            ConsoleIO.AddSubTaskSuccess();
        }

        private static void CompleteSubTask(int MainTaskID, string SubTaskName)
        {
            taskList[taskList.FindIndex(task => task.ID == MainTaskID)].CompleteSubTask(SubTaskName);
        ...
        {
            groupList.RemoveAt(groupList.FindIndex(group => group.idGroup == ID));
        ...
        private static void AddToGroup(int id, string name)
        {
            ...
            try
            {
                groupList[groupList.FindIndex(group => group.GroupName == name)].addTask(taskList[taskList.FindIndex(task => task.ID == id)]);
        ...
        private static void DeleteFromGroup(int idDelete, string name)
        {
            try
            {
                groupList[groupList.FindIndex(group => group.GroupName == name)].DeleteTask(idDelete);
