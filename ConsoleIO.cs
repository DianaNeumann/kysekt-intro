...
namespace sekta
{
    public class ConsoleIO
    {
        public static  void ShowTaskCMD(Task task)
        {
            ...
            Console.WriteLine($"Task: {task._taskName} " +
                $"\nID: {task._id} " +
                $"\nDate: {task._date}" +
                $"\nCompleted: {task._isCompleted}" +
                $"\nSubtasks: ");

            foreach (subTask miniTask in task._subTasks)
            {
        ...
        {
            Console.WriteLine($"ID: {group._idGroup} " + 
                $"\nGroup: {group._groupName} " +
                $"\nTasks in group: ");

            foreach (var task in group._groupList)
            {
                Console.Write("\t- " + task._taskName + " ID: " + task._id + "\n");
            }
        }
        ...
        public static void SuccessAdd(Task task)
        {
            ...
            Console.WriteLine($"[+] Task added. ID={task._id}");
            Console.ResetColor();
        }
